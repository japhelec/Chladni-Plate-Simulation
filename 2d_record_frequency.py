...
from vpython import *
import numpy as np
A, NN, omega = 0.4, 2025, 2*pi/1.0
size, m, k, d = 0.06, 0.1, 7500.0, 0.4
N = int(sqrt(NN))
middle = int((N+1)/2)
array_m = middle-1
t,dt = 0, 0.001
...
ballspos[array_m,array_m,1] = 0.06
...
while t<20:
    ...
    #ballspos[array_m][array_m][1] = A * sin(omega * t )
    ...
    ballsv[1:-1,1:-1,:] += rspringsacc[1:-1,1:,:] - rspringsacc[1:-1,0:-1,:] + cspringsacc[1:,1:-1,:] - cspringsacc[0:-1,1:-1,:]    
    ballspos[:,:,:] += ballsv[:,:,:]*dt
    ...
    if t > n*dt:
        for i in range(N):
            for j in range(N):
                balls[i][j].pos = vec(ballspos[i,j,0], ballspos[i,j,1]*300, ballspos[i,j,2])
        n += 10
    '''
